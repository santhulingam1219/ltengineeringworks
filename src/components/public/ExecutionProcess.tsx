...
export function ExecutionProcess() {
  const steps = [
    ...
    },
    {
      ...
      desc: "Schedule formulation, engineering review, equipment readiness, and methodology.",
      icon: Compass,
    },
    {
      number: "03",
      title: "Crew Mobilization",
      desc: "Rapid deployment of verified fitters, fabricators, riggers, and site supervisors.",
      icon: UsersThree,
    },
    {
      number: "04",
    ...
    <section className="py-12 sm:py-16 lg:py-20 bg-slate-900 text-white border-b border-slate-800 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <ScrollReveal direction="up">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-16 pb-4 sm:pb-6 border-b border-slate-800 gap-3 sm:gap-4">
            <div>
              <div className="inline-flex items-center gap-1.5 text-[10px] sm:text-xs font-mono font-bold text-amber-400 uppercase tracking-wider mb-1.5">
        ...
        {/* 8-Step Process Grid (2 Columns on Mobile, 4 Columns on Desktop) */}
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-6">
          {steps.map((step, index) => {
            const Icon = step.icon;
            return (
              <ScrollReveal key={index} delay={index * 50} direction="up" className="h-full">
                <div className="bg-slate-950/85 border border-slate-800 p-3.5 sm:p-6 rounded-xs space-y-2.5 sm:space-y-4 card-industrial-glow-dark transition-all group relative h-full flex flex-col justify-between shadow-xs">
                  <div className="space-y-2 sm:space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xs bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-400 group-hover:bg-amber-500 group-hover:text-slate-950 transition-colors shadow-xs">
                        <Icon className="w-4 h-4 sm:w-5 sm:h-5" weight="bold" />
                      </div>
                      <span className="text-lg sm:text-2xl font-heading font-black text-slate-600 group-hover:text-amber-400 transition-colors font-mono-code">
                        {step.number}
                      </span>
                    </div>

                    <div>
                      <h3 className="text-xs sm:text-base font-heading font-bold text-white uppercase tracking-tight group-hover:text-amber-400 transition-colors leading-snug">
                        {step.title}
                      </h3>
                      <p className="text-[10px] sm:text-xs text-slate-400 mt-1 sm:mt-2 leading-snug font-sans line-clamp-3 sm:line-clamp-none">
                        {step.desc}
                      </p>
                    </div>
                  </div>
